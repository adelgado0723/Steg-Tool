...
/**
  Decrpyts an image that was encrypted with a character string by this program.
...
int main(int argc, char **argv) {
	...
	PIXEL *bmp1;
	PIXEL *bmp2;
	static char usage[] = "usage: %s [-e | -d] [-m messageFile] [-o ouput_file] [-h] [image_file]\n";
	c = getopt(argc, argv, "deo:m:h");
	...
	do {
		switch (c) {
			...
			default:
				fprintf(stderr, "Invalid arguement provided. Use -h to see available options.\n");
				fprintf(stderr, usage, argv[0]);
